...
public class GridEntrust extends SpacesItemDecorationEntrust {
	...
	private final int currentChildPosition;

	public GridEntrust(int leftRight, int topBottom, int mColor, int currentChildPosition) {
	...
	public void getItemOffsets(Rect outRect, View view, final RecyclerView parent, RecyclerView.State state) {
		...
		if (childPosition < currentChildPosition) {//判断当前位置 指定位置开始画分割线
			return;
		}
		if (layoutManager.getOrientation() == GridLayoutManager.VERTICAL) {
			//判断是否在第一排
