package com.ffxz.cosmetics.util.luban;
...
import com.ffxz.cosmetics.listener.JudgeCompressListener;
...
public class Luban {
    ...
    private JudgeCompressListener judgeCompressListener;
    private File mFile;
    ...
    /**/
    public Luban launchJudge(JudgeGoodsData data) {
        Preconditions.checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
        ...
        if (gear == Luban.FIRST_GEAR)
            ...
            Observable.just(data)
                    .map(new Func1<JudgeGoodsData, JudgeGoodsData>() {
                        @Override
                        public JudgeGoodsData call(JudgeGoodsData data) {
                            LogUtils.i("img"+data.getImg());
                            data.setScaledImg(thirdCompress(data.getImg()));
                            LogUtils.i("datascale的abso"+data.getScaledImg()+"..data"+data.getScaledImg().getAbsolutePath());
                            return data;
                        }
                    })
                    .subscribeOn(Schedulers.io())
                    ...
                    .doOnError(new Action1<Throwable>() {
                        @Override
                        public void call(Throwable throwable) {
                            if (judgeCompressListener != null) judgeCompressListener.onError(throwable);
                        }
                    })
                    .onErrorResumeNext(Observable.<JudgeGoodsData>empty())
                    .filter(new Func1<JudgeGoodsData, Boolean>() {
                        @Override
                        public Boolean call(JudgeGoodsData data) {
                            return data != null;
                        }
                    })
                    .subscribe(new Action1<JudgeGoodsData>() {
                        @Override
                        public void call(JudgeGoodsData data) {
                            if (judgeCompressListener != null) judgeCompressListener.onJudgeSuccess(data);
                        }
                    });
    ...
    /**/
    public Luban setJudgeCompressListener(JudgeCompressListener listener) {
        judgeCompressListener = listener;
        return this;
    }
    ...
    private File compress(String largeImagePath, String thumbFilePath, int width, int height, int angle, long size) {
        ...
        LogUtils.i("thumbFilePath的值"+thumbFilePath);
