package com.ffxz.cosmetics.widget.CustomViewGroup;
...
import com.ffxz.cosmetics.model.GoodsSaleEntity;
 ...
 */
public class CustomSizeDialogViewGroup<X extends TextView> extends ViewGroup {
    ...
    private List<GoodsSaleEntity.GoodsSaleData.GoodsSaleList> mTexts = new ArrayList<>();
    private Context mContext;
    private int textModePadding;

    //正常样式
    private float itemTextSize = 14;
    private int itemBGResNor = R.drawable.shape_corner_ed_radius2;
    private int itemTextColorNor = Color.parseColor("#101010");

    //选中的样式
    private int itemBGResPre = R.drawable.shape_corner_e8_radius2;
    private int itemTextColorPre = Color.parseColor("#FFFFFF");

    private int position ;

    public CustomSizeDialogViewGroup(Context context) {
        this(context, null);
        init();
    }

    public CustomSizeDialogViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    private void init(){
        ...
        HorInterval = Math.round(12*ratio);
...
//                posLeft = HorInterval;
            ...
            if(viewWidth - posLeft >= measuredWidth) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            childView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    onGroupItemClickListener.onGroupItemClick(itemPos,getChooseText(itemPos),position);
    ...
    public void addItemViews(List<GoodsSaleEntity.GoodsSaleData.GoodsSaleList> texts, String mode,int position) {
        mTexts = texts;
        this.position = position;
        removeAllViews();
        for(int i = 0 ;i<texts.size();i++){
            addItemView(texts.get(i).getProvalue(),mode);
        }
    }
    ...
    public String getChooseText(int itemID) {
        ...
        if (itemID >= 0) {
            return mTexts.get(itemID).getProvalue();
        }
        return null;
    }
    ...
    public interface OnGroupItemClickListener {
        void onGroupItemClick(int item, String text, int position);
    }
}