 ...
 */
public class CustomSearchHotViewGroup<X extends TextView> extends ViewGroup {
    ...
    //选中的样式
    private int itemBGResPre = R.drawable.shape_corner_fb_radius3;
    private int itemTextColorPre = Color.parseColor("#101010");


    public CustomSearchHotViewGroup(Context context) {
        this(context, null);
        init();
    }

    public CustomSearchHotViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    private void init(){
        ...
        HorInterval = Math.round(16*ratio);  //水平间隔
        VerInterval= Math.round(10*ratio);    //垂直间隔
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            childView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    onGroupItemClickListener.onHotGroupItemClick(itemPos,getChooseText(itemPos));
//                    chooseItemStyle(itemPos);
    ...
    public void addItemViews(List<String> texts, String mode) {
        ...
        for(int i = 0 ;i<texts.size();i++){
            addItemView(texts.get(i),mode,i);
        }
    }

    private void addItemView(String text, String mode,int position) {
        X childView = null;
        switch (mode) {
            case BTN_MODE:
        ...
        if(position == 0 ){
            childView.setBackgroundResource(R.drawable.shape_corner_fb_radius3);
            Drawable drawable= getResources().getDrawable(R.mipmap.fire_sear);
            drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
            childView.setCompoundDrawables(drawable,null,null,null);//设置TextView的drawableleft
            childView.setCompoundDrawablePadding(12);//设置图片和text之间的间距
        }else {
            childView.setBackgroundResource(itemBGResNor);
        }
        setItemPadding(childView);
    ...
    public interface OnGroupItemClickListener {
        void onHotGroupItemClick(int item, String text);
    }
}