...
public class HomeGoodsListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	public HomeGoodsListAdapter(Context mContext, List<GoodsListEntity.DataBean.ProductListBean> mList, GridLayoutManager layoutManager) {
	...
	public void setItemData(CollectionViewHolder holder, int position) {
		...
		holder.tvJudge.setText("销量:" + mList.get(position).getProduct_sales());
//		holder.tvReturn.setText("￥" + mList.get(position).getProduct_orginal());
		if (mList.get(position).getShop_name() != null) {
			holder.tvStore.setVisibility(View.VISIBLE);
			holder.tvStore.setText(mList.get(position).getShop_name());
		} else {
