...
import com.ffxz.cosmetics.model.SeckillEntity;
import com.ffxz.cosmetics.util.LogUtils;
import com.ffxz.cosmetics.widget.CustomCountDownTimerViews;
...
public class HomeSeckillActivityAdapters extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	public HomeSeckillActivityAdapters(Context mContext, List<SeckillEntity.SeckillData.SeckillList> mList, int flag) {
	...
	public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		if (viewType == 0) {
			...
			seckillItemViewHolder = new SeckillItemViewHolder(MaterialRippleView.create(LayoutInflater
					.from(mContext).inflate(R.layout.item_home_activity_seckill_item_, parent, false)), mItemClickListener);
			...
			seckillCommingViewHolder = new SeckillCommingViewHolder(MaterialRippleView.create(LayoutInflater
					.from(mContext).inflate(R.layout.item_home_seckill_comming_, parent, false)), mItemClickListener);
	...
	public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
		if (holder instanceof SeckillItemViewHolder) {
			...
			if(position == 1){
				((SeckillItemViewHolder) holder).v1.setVisibility(View.VISIBLE);
			}else{
				((SeckillItemViewHolder) holder).v1.setVisibility(View.GONE);
			}
...
//
			...
			if(position == 1){
				((SeckillCommingViewHolder) holder).v1.setVisibility(View.VISIBLE);
			}else{
				((SeckillCommingViewHolder) holder).v1.setVisibility(View.GONE);
			}
	...
	class SeckillItemViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
		...
		CustomCountDownTimerViews tvTimer;
