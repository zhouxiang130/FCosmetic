...
import static com.ffxz.cosmetics.util.PermissionUtils.READ_EXTERNAL_STORAGE;
import static com.ffxz.cosmetics.util.PermissionUtils.REQUEST_CODE_READ_PHONE_STATE;
import static com.ffxz.cosmetics.util.PermissionUtils.REQUEST_CODE_WRITE_EXTRONAL_STORAGE;
...
public class MainActivity extends BaseActivity {
	...
	private Fragment mHomepage, mClassify, mCart, mMine1;
	...
	protected void initView() {
		...
		mTextView = new TextView[]{tvHomepage, tvClassify, tvCart, tvMine};
	...
	private void initFragment() {
		mFragments = new Fragment[]{mMine1, mClassify, mCart, mHomepage};
		...
		mManager.beginTransaction()
				...
				.show(mMine1)
				.commit();
		mManager.beginTransaction().hide(mClassify).hide(mCart).hide(mHomepage).commitAllowingStateLoss();
		mCurrent = mMine1;
		if (mPage != 0) {
			selected(mPage);
		}
	...
	@OnClick({R.id.main_tv_homepage, R.id.main_tv_classify, R.id.main_tv_cart, R.id.main_tv_mine})
...
//                showDialog("检测到您未开启读写功能,这将对您使用本应用带来极大的不便,我们强烈建议您开启此功能,点击确认手动更改");
	...
	private void isReadPermissionAllowed() {
		if (PermissionUtils.isPermissionAllowed(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
			...
			//没有相关权限
			if (PermissionUtils.isRemindAllowed(this, Manifest.permission.READ_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE)) {
				//是否能弹窗提示
